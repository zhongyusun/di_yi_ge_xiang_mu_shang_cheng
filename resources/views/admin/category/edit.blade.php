            ...
            <div class="col-lg-12">
                <div class="card">
                    ...
                    <ul class="nav nav-tabs customtab" role="tablist">
                        ...
                        <li class="nav-item">
                            <a class="nav-link active" href="{{route('admin.category.edit',$category)}}">
                                <span class="hidden-sm-up"><i class="ti-user"></i></span> <span class="hidden-xs-down">编辑分类</span>
                            </a>
                        </li>
                    </ul>
                    <!-- Tab panes -->
                        ...
                        <div class="card-body">
                            <form action="{{route('admin.category.update',$category)}}" method="post" class="form-horizontal ">
                                ...
                                <div class="form-group row">
                                    ...
                                    <div class="col-md-9">
                                        <input type="text" name="title" required value="{{$category->title}}" placeholder="请输入栏目名称" class="form-control">
                                    </div>
                                </div>
                                <div class="form-group row">
                                    <label class="control-label text-right col-md-3">父级分类</label>
                                    <div class="col-md-9">
                                        <select class="form-control custom-select" name="parent_id" required data-placeholder="Choose a Category" tabindex="1">
                                            <option value="0">顶级分类</option>
                                            @foreach($categories as $v)
                                                <option @if($category['parent_id']==$v['id']) selected @endif value="{{$v['id']}}" >{!! $v['_title'] !!}</option>
